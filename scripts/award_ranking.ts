...
import type { IArchivesLog, IAwardRanking } from './typings';
import { DATE_FORMAT_TEMPLATE, getISOWeekNumber, getWeekStartAndEnd } from './utils';
...
// 当天日期
const now = dayjs();
const today = now.format(DATE_FORMAT_TEMPLATE);
const yesterday = now.subtract(1, 'day').format(DATE_FORMAT_TEMPLATE);
...
export const awardRanking = () => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (award) {
      ...
      if (userAward) {
        userAward.weeks = Array.from(new Set([...(userAward.weeks || []), curISOWeekNumber]));
      } else {
